...
initiated by the user. When this happpens, the client encrypts its
secret key material with a secure symmetric code, then places the
result in the user's inbox by sending it as a mail addressed to
themself.
...
The symmetric encryption key MUST be generated by the implementation
itself using a CSPRNG, and presented to the user for safekeeping. The
backup code format SHOULD be 24 uppercase alphanumeric characters,
divided into six blocks of four, separated by dashes. This format
holds 124 bits of data, it is designed to be unambiguous,
...
It is RECOMMENDED to ask the user to input the key again after
display. While this is surely an annoyance, it ensures that the code
was actually written down and the backup is not rendered useless.

The implementation SHOULD use the salted+iterated S2K algorithm to
derive a key from the backup code. While the use of a memory-hard KDF
like scrypt or argon2 would be desirable in the future, this is not
...
secret keys are never stuck on the device they were generated on, but
...
.. todo::
   ...
   - What should we name this code? I picked backup code because
     that's what it's called in OpenKeychain at the moment, but
     another name may be more appropriate here.
