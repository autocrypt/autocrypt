...
- The payload MUST contain a single ASCII-armored block of OpenPGP
  ...
  explanations as discussed in :doc:`suggestions for key-transfer format<transfer-format>`.
 ...
 * The client should prompt the user for their corresponding Setup
   Code.  If there is an ``Autocrypt-Setup-Message: v0`` header,
   the client may choose to present the user with a
